...
public:
    ...
    bool SaveBitmap(const TCHAR* path) {
...
//                        __asm nop // should never happen!
    ...
    {
        int i=0;
        for(auto point=points_.cbegin();point!=points_.cend();++point)
        {
            int x=points_[i].x;
            int y=points_[i].y;
            i++;
    ...
    void CreatePoints(int count) {
        ...
        {
            Point pointtest;
            pointtest.x=rand()%w+10;
            pointtest.y=rand()%h+10;
            points_.push_back(pointtest);
        }
    }
